...
class ArcaLiveCrawlerPlaywright:
    ...
    async def crawl_board(
        self, board_id: str = None, page: int = 1, max_pages: int = None, crawl_session_id: str = None, hours_ago: int = 1
    ) -> List[CommunityPost]:
        """게시판 크롤링 (1시간 전 게시물부터 최신까지)"""
        ...
        try:
            ...
            for current_page in range(1, max_pages + 1):
                if found_cutoff:
                    print(f"⏹️  수집 종료 (페이지 {current_page-1}까지)")
                    break

                print(f"\n📄 페이지 {current_page} 크롤링 중...")

                posts = await self._crawl_page(context, board_id, current_page, crawl_session_id)
            ...
            print(f"\n✅ 총 {len(all_posts)}개 게시글 크롤링 완료 (최근 {hours_ago}시간)")
    ...
    async def _crawl_page(self, context, board_id: str, page: int, crawl_session_id: str) -> List[CommunityPost]:
        """단일 페이지 크롤링"""
        ...
        try:
            ...
            posts = await self._parse_post_list_with_details(context, soup, board_id, crawl_session_id)
    ...
    async def _parse_post_list_with_details(self, context, soup: BeautifulSoup, board_id: str, crawl_session_id: str) -> List[CommunityPost]:
        """게시글 목록 파싱 + 본문/댓글 크롤링"""
        posts = []
